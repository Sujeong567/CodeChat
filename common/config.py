...
LORA_ALPHA = R_RANK * 2                  # LoRA alpha (스케일링 팩터)
LORA_TARGET_MODULES = ["q_proj", "v_proj"]      # LoRA를 적용할 레이어 목록
REPRESENTATIVE_LORA_TARGET_MODULE = "q_proj"    # xL을 대표로 캡처할 모듈
LORA_INJECTION_MODULES = ["q_proj", "v_proj"]   # 델타를 주입할 모든 모듈
...
# --- LLM 추론 설정 ---
MAX_GEN_LENGTH = 100 # 각 요청에 대해 생성할 최대 토큰 수
SCALE_INJECTION = 5e-3 # FHE LoRA 델타를 LM Head 입력에 주입할 스케일 팩터 (매우 작게 설정하여 미세한 영향만 주도록)
